...
- return: 
        in case of mode = 'ok', try to create a sql sentence, then the return is tuple of (ivmnize::boolean, json)
...
function createApiSelectSentence(json_d, mode::String)
    ...
    if mode != "pre"
        ...
        if ck[1]
            ...
            return (ivmnize, json(Dict("result" => false, "resembled" => ck[2])))
        else
            # yes this is the new
            ...
            if ret[1]
                ...
                return (ivmnize, json(Dict("result" => true, "apino" => ret[2])))
            else
                return (ivmnize, ret[1])
            end
        end
    else
